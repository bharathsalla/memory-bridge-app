...
export default function TabBar() {
  ...
  if (isCaregiverView) {
    return (
      <div className="bg-card/95 backdrop-blur-md border-t border-border/15 shrink-0">
          ...
          {caregiverTabs.map(tab => {
            ...
            return (
              <button key={tab.id} onClick={() => setActiveCaregiverTab(tab.id)}
                ...
                {active && (
                  <motion.div layoutId="cg-tab-bg" className="absolute inset-0 bg-primary/6" transition={{ type: 'spring', bounce: 0.2, duration: 0.4 }} />
                )}
        ...
        {tabs.map(tab => {
          ...
          return (
            <button key={tab.id} onClick={() => setActivePatientTab(tab.id)}
              ...
              {active && (
                <motion.div layoutId="patient-tab-bg" className="absolute inset-0 bg-primary/6" transition={{ type: 'spring', bounce: 0.2, duration: 0.4 }} />
              )}
