...
export default function OnboardingScreen() {
  ...
  return (
    <div className="h-full bg-background flex flex-col lavender-shimmer">
      {/* Progress dots */}
      ...
      <AnimatePresence mode="wait">
        <motion.div key={step} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.3 }} className="flex-1 flex flex-col px-7">
          ...
          {step === 'welcome' && (
            <div className="flex-1 flex flex-col items-center justify-center text-center">
              <motion.div initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.2, type: 'spring', bounce: 0.4 }}
                className="w-32 h-32 overflow-hidden mb-8 shadow-lg">
                <img alt="MemoCare" className="w-full h-full object-cover shadow-none" src="/lovable-uploads/e870b19c-fc1d-486f-815f-86f58cef95ed.png" />
              </motion.div>
              ...
              <div className="mt-auto mb-8 w-full space-y-3">
                <button onClick={next} className="w-full h-14 bg-primary text-primary-foreground text-[17px] font-bold active:scale-[0.98] transition-transform shadow-md">
                  Get Started
                </button>
                <button onClick={() => { setSelectedMode('full'); setStep('personalize'); }} className="w-full h-12 text-primary text-[16px] font-semibold">
                  I'm a caregiver
                </button>
              </div>
            </div>
          ...
          {step === 'voiceChoice' && (
            <div className="flex-1 flex flex-col items-center justify-center text-center">
              <motion.div initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.1, type: 'spring', bounce: 0.4 }}
                className="w-24 h-24 bg-gradient-to-br from-secondary/20 to-secondary/5 flex items-center justify-center mb-8">
              ...
              <div className="w-full space-y-3">
                <motion.button whileTap={{ scale: 0.97 }} onClick={handleVoiceChoice}
                  ...
                  <div className="w-14 h-14 bg-secondary/10 flex items-center justify-center shrink-0">
            ...
            <div className="flex-1 flex flex-col">
              ...
              <div className={`ios-card-elevated overflow-hidden transition-all duration-300 ${
                ...
                <input id="onboarding-name" type="text" value={name} onChange={(e) => setName(e.target.value)}
                  ...
                  className="w-full h-14 px-5 bg-transparent text-ios-body text-foreground placeholder:text-muted-foreground/60 outline-none" autoFocus />
              </div>
              ...
              <div className="mt-auto mb-8">
                <button onClick={() => setStep('complete')} className="w-full h-14 bg-primary text-primary-foreground text-[17px] font-bold active:scale-[0.98] transition-transform shadow-md">
                  Continue
                </button>
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center">
              <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', bounce: 0.5 }}
                ...
                <motion.div initial={{ scale: 0, rotate: -45 }} animate={{ scale: 1, rotate: 0 }} transition={{ delay: 0.3, type: 'spring' }}
                  className="text-success text-5xl font-bold">✓</motion.div>
              </motion.div>
              ...
              <div className="mt-auto mb-8 w-full space-y-3">
                <button onClick={() => finish(true)} className="w-full h-14 bg-secondary text-secondary-foreground text-[17px] font-bold active:scale-[0.98] transition-transform shadow-md flex items-center justify-center gap-3">
                  <Mic className="w-5 h-5" />
                  Start with Voice Care
                </button>
                <button onClick={() => finish(false)} className="w-full h-14 bg-primary text-primary-foreground text-[17px] font-bold active:scale-[0.98] transition-transform shadow-md">
                  Start Browsing
                </button>
              </div>
            </div>
