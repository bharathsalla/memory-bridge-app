  ...
  return (
    <div className="h-full overflow-y-auto bg-background pb-6">
      ...
      <div className="px-5">
        <div className="ios-card-elevated p-5 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-[14px] text-muted-foreground">Tap to edit profile</div>
          </div>
      ...
      <div className="px-5 mt-6">
        <h2 className="text-[18px] font-bold text-foreground mb-3">How are you feeling?</h2>
        <div className="ios-card-elevated p-5">
          ...
          {currentMood && (
            <div className="mt-3 pt-3 border-t border-border/40 text-center text-[13px] text-muted-foreground">
      ...
      {/* Health Summary */}
      <div className="px-5 mt-6">
        <h2 className="text-[18px] font-bold text-foreground mb-3">Health Summary</h2>
        <div className="grid grid-cols-2 gap-3">
          {[
            { label: 'Sleep', value: `${sleepHours}h`, Icon: Moon, trend: 'Good', trendColor: 'text-success' },
            { label: 'Steps', value: stepCount.toLocaleString(), Icon: Heart, trend: 'Normal', trendColor: 'text-primary' },
            { label: 'Med Adherence', value: `${medicationAdherence}%`, Icon: Shield, trend: 'Excellent', trendColor: 'text-success' },
            { label: 'Mood Trend', value: '😊', Icon: Heart, trend: 'Stable', trendColor: 'text-accent' },
          ].map(stat => (
            <div key={stat.label} className="ios-card-elevated rounded-2xl p-5 flex flex-col gap-2">
              ...
              <div className="text-[12px] text-muted-foreground font-medium uppercase tracking-wide">{stat.label}</div>
              <div className={`text-[13px] font-semibold ${stat.trendColor}`}>{stat.trend}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Mode Switcher */}
      <div className="px-5 mt-6">
        <h2 className="text-[18px] font-bold text-foreground mb-3">Interface Mode</h2>
        <button
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-[15px] text-foreground font-semibold">Current: {modes.find(m => m.id === mode)?.label}</div>
          ...
          >
            {modes.map(m => (
              <motion.button
                ...
                className={`w-full p-4 rounded-2xl flex items-center gap-3 text-left transition-all touch-target ios-card-elevated ${
      ...
      {/* Settings List */}
      <div className="px-5 mt-6">
        <h2 className="text-[18px] font-bold text-foreground mb-3">Settings</h2>
          ...
          {[
            ...
            return (
              <button key={item.label} className="w-full flex items-center gap-3 p-4 text-left active:bg-muted/20 transition-colors touch-target">
                ...
                <span className="text-[15px] text-foreground flex-1 font-medium">{item.label}</span>
