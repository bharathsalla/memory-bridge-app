...
export default function TodayScreen() {
  ...
  if (mode === 'essential') {
    return (
      <div className="h-full flex flex-col items-center justify-center px-6 text-center bg-gradient-to-b from-background to-surface">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 w-full">
          <div>
            <img src={patientAvatar} alt="Profile" className="w-28 h-28 rounded-[2rem] mx-auto mb-5 object-cover ring-4 ring-primary/25 shadow-lg" />
            <h1 className="text-[46px] font-extrabold text-foreground leading-none tracking-tight">{greeting()}</h1>
          ...
          <motion.button whileTap={{ scale: 0.96 }} onClick={() => setShowIDCard(true)}
            className="w-full py-5 rounded-3xl bg-primary text-primary-foreground flex items-center justify-center gap-3 shadow-lg">
          ...
          {pendingMeds.length > 0 ? (
            <motion.button whileTap={{ scale: 0.96 }} onClick={() => markMedicationTaken(pendingMeds[0].id)}
              className="w-full py-8 rounded-3xl bg-gradient-to-br from-accent to-warning text-accent-foreground flex flex-col items-center justify-center gap-3 shadow-lg">
              <img src={medImages[pendingMeds[0].name] || defaultMedImg} alt={pendingMeds[0].name}
                className="w-20 h-20 rounded-2xl object-cover border-3 border-white/30 shadow-md" onError={e => { (e.target as HTMLImageElement).src = defaultMedImg; }} />
            ...
            <div className="w-full py-8 rounded-3xl bg-success/10 border-2 border-success/30 flex flex-col items-center justify-center gap-3">
              <Check className="w-16 h-16 text-success" />
              <span className="text-[26px] font-extrabold text-success">All done for now</span>
            </div>
          )}

          <button className="w-full h-[72px] rounded-3xl bg-destructive text-destructive-foreground text-[26px] font-extrabold sos-pulse shadow-lg">
            🆘 Emergency Call
          </button>
        </motion.div>
  ...
  if (mode === 'simplified') {
    return (
      <div className="h-full overflow-y-auto bg-gradient-to-b from-background to-surface pb-4">
        <div className="px-5 pt-5 pb-3">
          <div className="ios-card-elevated p-5 flex items-center gap-4 rounded-3xl">
            <img src={patientAvatar} alt="Profile" className="w-[68px] h-[68px] rounded-2xl object-cover shrink-0 ring-3 ring-primary/20 shadow-md" />
            <div>
        ...
        <div className="px-5 space-y-3">
          ...
          <motion.button whileTap={{ scale: 0.97 }} onClick={() => setShowIDCard(true)}
            className="w-full ios-card-elevated p-5 flex items-center gap-4 text-left rounded-2xl border-2 border-primary/20 bg-primary/5">
            <div className="w-14 h-14 rounded-2xl bg-primary/15 flex items-center justify-center shrink-0">
              <User className="w-7 h-7 text-primary" />
            </div>
            <div className="flex-1">
              <div className="text-[22px] font-extrabold text-primary">View Me</div>
              <div className="text-[16px] text-muted-foreground font-medium">My details & contacts</div>
            </div>
          ...
          {pendingMeds.map(med => (
            <motion.button key={med.id} whileTap={{ scale: 0.97 }} onClick={() => markMedicationTaken(med.id)}
              ...
              <img src={medImages[med.name] || defaultMedImg} alt={med.name}
                className="w-16 h-16 rounded-2xl object-cover shrink-0 shadow-sm border border-border/30" onError={e => { (e.target as HTMLImageElement).src = defaultMedImg; }} />
              ...
              <div className="px-5 py-3 rounded-2xl bg-primary text-primary-foreground text-[17px] font-extrabold shrink-0 shadow-sm">
                Take
              </div>
            </motion.button>
          ))}
  ...
  return (
    <div className="h-full relative">
      <div className="h-full overflow-y-auto bg-gradient-to-b from-background via-background to-surface pb-20">
        {/* Greeting */}
        <div className="px-5 pt-5 pb-3">
          <div className="ios-card-elevated p-4 flex items-start justify-between rounded-2xl">
            <div className="flex items-center gap-3.5">
              <img src={patientAvatar} alt="Profile" className="w-14 h-14 rounded-2xl object-cover shrink-0 ring-2 ring-primary/20 shadow-sm" />
              <div>
                <p className="text-[14px] text-muted-foreground font-bold uppercase tracking-wider">{greeting()}</p>
          ...
          <div className="grid grid-cols-4 gap-2">
            {[
              ...
              <motion.div key={stat.label} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                className="ios-card-elevated rounded-2xl p-3 flex flex-col items-center gap-1.5">
                {stat.Icon ? (
        ...
        <div className="px-5 mt-4">
          <motion.button whileTap={{ scale: 0.97 }} onClick={() => setShowIDCard(true)}
            className="w-full ios-card-elevated p-4 flex items-center gap-3.5 rounded-2xl bg-gradient-to-r from-primary/8 to-accent/8 border-2 border-primary/15">
            <div className="w-12 h-12 rounded-xl bg-primary/15 flex items-center justify-center shrink-0">
              <User className="w-6 h-6 text-primary" />
            </div>
            <div className="flex-1 text-left">
              <div className="text-[17px] font-extrabold text-primary">View Me</div>
              <div className="text-[13px] text-muted-foreground font-semibold">My ID, contacts & details</div>
            </div>
          ...
          <div className="space-y-2.5">
            {medications.map((med, i) => (
              <motion.div key={med.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                ...
                <img src={medImages[med.name] || defaultMedImg} alt={med.name}
                  className={`w-14 h-14 rounded-xl object-cover shrink-0 shadow-sm border border-border/30 ${med.taken ? 'grayscale opacity-50' : ''}`}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[13px] text-muted-foreground mt-0.5 flex items-center gap-1 font-semibold">
                ...
                {med.taken ? (
                  ...
                  <button onClick={() => markMedicationTaken(med.id)}
                    className="px-5 h-11 rounded-xl bg-primary text-primary-foreground text-[15px] font-extrabold active:scale-95 transition-transform touch-target shadow-sm">
                    Take
                  </button>
                )}
              </motion.div>
              ...
              {activities.map((item, i) => (
                <div key={item.id} className="flex items-start gap-3 py-3">
                  ...
                  <div className="flex-1 pt-1">
                    <div className={`text-[15px] font-semibold leading-snug ${item.completed ? 'text-foreground' : 'text-muted-foreground'}`}>
                      {item.description}
                    </div>
      ...
      {/* Voice FAB */}
      <button className="absolute bottom-5 right-5 w-14 h-14 rounded-2xl bg-primary text-primary-foreground flex items-center justify-center active:scale-90 transition-transform z-30 shadow-lg" aria-label="Voice input">
