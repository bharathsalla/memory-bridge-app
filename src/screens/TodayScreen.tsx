...
export default function TodayScreen() {
  ...
  if (mode === 'essential') {
    return (
      <div className="h-full flex flex-col items-center justify-center px-6 text-center bg-background lavender-shimmer">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 w-full">
          ...
          {pendingMeds.length > 0 ? (
            <motion.button whileTap={{ scale: 0.96 }} onClick={() => markMedicationTaken(pendingMeds[0].id)}
              className="w-full py-10 bg-secondary text-secondary-foreground flex flex-col items-center justify-center gap-4">
              <img src={medImages[pendingMeds[0].name] || defaultMedImg} alt={pendingMeds[0].name}
                className="w-24 h-24 object-cover border-2 border-secondary-foreground/20" onError={(e) => {(e.target as HTMLImageElement).src = defaultMedImg;}} />
  ...
  return (
    <div className="h-full relative">
      <div className="h-full overflow-y-auto bg-background warm-glow pb-24">
        {/* Greeting */}
        <div className="px-5 pt-5 pb-4">
          <div className="ios-card-elevated p-5 flex items-start justify-between my-[6px] px-[17px] mx-[9px]">
            <div className="flex items-center gap-4">
              <img src={patientAvatar} alt="Profile" className="w-16 h-16 object-cover shrink-0 ring-2 ring-secondary/25" />
              <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <ModeBadge />
              <button onClick={toggleCaregiverView} className="w-12 h-12 bg-primary/8 flex items-center justify-center touch-target" aria-label="Open caregiver view">
          ...
          <div className="grid grid-cols-2 gap-3 px-[11px]">
            {[
              ...
              <motion.div key={stat.label} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                className="ios-card-elevated p-4 flex flex-col items-center gap-2 py-[19px] px-[20px] my-[2px]">
