    ...
    return (
      <div className="h-full flex flex-col items-center justify-center px-6 text-center bg-background">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 w-full">
          <div>
            <img src={patientAvatar} alt="Profile" className="w-32 h-32 rounded-[2rem] mx-auto mb-5 object-cover ring-4 ring-primary/20" />
          ...
          <motion.button whileTap={{ scale: 0.96 }} onClick={() => setShowIDCard(true)}
            className="w-full py-6 rounded-3xl bg-primary text-primary-foreground flex items-center justify-center gap-4">
          ...
          {pendingMeds.length > 0 ? (
            <motion.button whileTap={{ scale: 0.96 }} onClick={() => markMedicationTaken(pendingMeds[0].id)}
              className="w-full py-10 rounded-3xl bg-accent text-accent-foreground flex flex-col items-center justify-center gap-4">
              <img src={medImages[pendingMeds[0].name] || defaultMedImg} alt={pendingMeds[0].name}
                className="w-24 h-24 rounded-2xl object-cover border-2 border-accent-foreground/20" onError={e => { (e.target as HTMLImageElement).src = defaultMedImg; }} />
            ...
            <div className="w-full py-10 rounded-3xl bg-success/10 flex flex-col items-center justify-center gap-4">
              <Check className="w-18 h-18 text-success" />
              <span className="text-[30px] font-extrabold text-success">All done for now</span>
            </div>
          )}

          <button className="w-full h-[80px] rounded-3xl bg-destructive text-destructive-foreground text-[28px] font-extrabold sos-pulse">
            🆘 Emergency Call
          </button>
        </motion.div>
      ...
      <div className="h-full overflow-y-auto bg-background pb-6">
        ...
        <div className="px-5 space-y-4">
          <motion.button whileTap={{ scale: 0.97 }} onClick={() => setShowIDCard(true)}
            className="w-full ios-card-elevated p-6 flex items-center gap-5 text-left rounded-2xl bg-primary text-primary-foreground">
          ...
          {pendingMeds.map(med => (
            <motion.button key={med.id} whileTap={{ scale: 0.97 }} onClick={() => markMedicationTaken(med.id)}
              className="w-full ios-card-elevated p-5 flex items-center gap-5 text-left touch-target-xl rounded-2xl">
              ...
              <div className="px-6 py-3.5 rounded-2xl bg-primary text-primary-foreground text-[18px] font-extrabold shrink-0">Take</div>
            </motion.button>
          ))}

          {takenMeds.map(med => (
            <div key={med.id} className="w-full ios-card-elevated p-5 flex items-center gap-5 opacity-50 rounded-2xl">
              <img src={medImages[med.name] || defaultMedImg} alt={med.name}
  ...
  return (
    <div className="h-full relative">
      <div className="h-full overflow-y-auto bg-background pb-24">
        {/* Greeting */}
        <div className="px-5 pt-5 pb-4">
          <div className="ios-card-elevated p-5 flex items-start justify-between rounded-2xl">
            ...
            <div className="flex items-center gap-2 mt-1">
              <ModeBadge />
              <button onClick={toggleCaregiverView} className="w-12 h-12 rounded-xl bg-primary/10 flex items-center justify-center touch-target" aria-label="Open caregiver view">
          ...
          <div className="grid grid-cols-4 gap-3">
            {[
              ...
              <motion.div key={stat.label} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                className="ios-card-elevated rounded-2xl p-4 flex flex-col items-center gap-2">
        ...
        <div className="px-5 mt-5">
          <motion.button whileTap={{ scale: 0.97 }} onClick={() => setShowIDCard(true)}
            className="w-full p-5 flex items-center gap-4 rounded-2xl bg-primary text-primary-foreground">
              ...
              <motion.div key={med.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                ...
                {med.taken ? (
                  ...
                  <button onClick={() => markMedicationTaken(med.id)}
                    className="px-6 h-12 rounded-xl bg-primary text-primary-foreground text-[17px] font-extrabold active:scale-95 transition-transform touch-target">
                    Take
                  </button>
                )}
              </motion.div>
            ...
            {activities.map((item, i) => (
              <div key={item.id} className="flex items-start gap-4 py-3.5">
                <div className="flex flex-col items-center">
                  <div className={`w-13 h-13 rounded-xl flex items-center justify-center text-[18px] ${item.completed ? 'bg-success/10' : 'bg-muted'}`} style={{ width: 52, height: 52 }}>
                    {item.icon}
                  </div>
      ...
      <button className="absolute bottom-6 right-5 w-16 h-16 rounded-2xl bg-primary text-primary-foreground flex items-center justify-center active:scale-90 transition-transform z-30 ios-card-elevated" aria-label="Voice input">
