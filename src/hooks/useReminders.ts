...
export function useSendCaregiverReminder() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (payload: { type: string; message: string; photoUrl?: string; caregiverName: string; medName?: string; medDosage?: string; medQty?: string; medInstructions?: string }) => {
